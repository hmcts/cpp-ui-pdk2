...
export type FormErrorFormat = 'default' | 'extended';

@Component({
  selector: 'form[pdk-form], [formGroup][pdk-form]',
  exportAs: 'pdkForm',
  template: `<ng-content></ng-content>`
})
export class PdkFormComponent implements AfterContentInit, OnDestroy {
  @Input() errorFormat: FormErrorFormat = 'default';
  @Output() errors = new EventEmitter<ValidationError[] | null>();
  @Output() validSubmit = new EventEmitter<unknown>();

  reset$ = new Subject<void>();
  onBeforeSubmit$ = new Subject<void>();
